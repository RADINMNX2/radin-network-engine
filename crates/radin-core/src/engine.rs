...
impl RouteEngine {
    ...
    pub fn on_edge_observation(
        ...
        if let Some((loss_ratio, jitter, latency)) = snapshot {
            ...
            if self.cfg.loss_spike_threshold.is_sign_positive()
                && loss_ratio >= self.cfg.loss_spike_threshold
            {
                self.maybe_emit_spike(
                    EventKind::PacketLossSpike,
                    now,
                    "sustained loss above threshold",
                );
            }
        ...
        let mut candidates = self
            .registry
            .route_candidates(&self.cfg.supported_transports, MeasuredSource::Real);
        score_all(&self.cfg.score_weights, &mut candidates, false)?;
        candidates.retain(|c| {
            self.breaker_for(c).state == crate::circuit::CircuitState::Closed || {
        ...
        match verdict {
            RouteVerdict::Switch {
                ...
                if let Some(target) = target {
                    self.decider
                        .apply_cooldown(&candidate_id, now + self.cfg.hysteresis.route_cooldown_ms);
    ...
    fn apply_selected(&mut self, edge_id: String, candidate: &RouteCandidate, now: TimestampMs) {
        ...
        let transport_changed =
            self.current.as_ref().map(|c| c.transport) != Some(candidate.transport);
        let edge_changed =
            self.current.as_ref().map(|c| route_edge_id(&c.id)) != Some(edge_id.as_str());
        ...
        if transport_changed && !first {
            ...
            self.emit_kind(
                EventKind::TransportChanged,
                now,
                Severity::Info,
                "transport switched by score",
            );
        }
    ...
    pub fn report_transport_failure(&mut self, transport: TransportKind, now: TimestampMs) {
        ...
        if newly_open {
            ...
            self.emit_kind(
                ...
                now,
                Severity::Warning,
                "breaker opened on transport failure",
            );
        }
    ...
    pub fn may_forward_direct(&self) -> bool {
        matches!(
            self.cfg.data_plane_policy,
            DataPlanePolicy::AllowDirectFallback
        )
    }
    ...
    pub fn breaker_for(&mut self, candidate: &RouteCandidate) -> &mut CircuitBreaker {
        ...
        self.breakers
            .entry(key.clone())
            .or_insert_with(|| CircuitBreaker::new(key, self.cfg.circuit.clone()))
    }
    ...
    pub fn diagnostic_report(
        &self,
        now: TimestampMs,
    ) -> crate::Result<crate::telemetry::DiagnosticReport> {
...
mod tests {
    ...
    fn engine_selects_best_edge_on_first_observation() {
        ...
        e.set_edges(
            vec![
                edge("sg", "ap-sg", "10.0.0.1"),
                edge("eu", "eu-west", "10.0.0.2"),
            ],
            0,
        )
        .unwrap();
        // Stable SG edge.
        for i in 0..10 {
            e.on_edge_observation(&obs("sg", 40.0, 2.0, false), i)
                .unwrap();
        }
        // Jittery EU edge.
        for i in 0..10 {
            e.on_edge_observation(&obs("eu", 38.0, 35.0, i % 4 == 0), i)
                .unwrap();
        }
        assert_eq!(
    ...
    fn engine_does_not_flap_on_tiny_latency_gap() {
        let mut e = engine();
        e.set_edges(
            vec![edge("a", "ap", "10.0.0.3"), edge("b", "na", "10.0.0.4")],
            0,
        )
        .unwrap();
        // a: 80ms stable. b: 77ms (tiny improvement, no jitter/loss benefit).
        for i in 0..5 {
            e.on_edge_observation(&obs("a", 80.0, 2.0, false), i)
                .unwrap();
        }
        let route_before = e.current_route().map(|c| c.id.clone());
        for i in 5..12 {
            e.on_edge_observation(&obs("b", 77.0, 2.0, false), i)
                .unwrap();
        }
    ...
    fn engine_falls_to_direct_when_edges_die_and_allows_fail_safe() {
        ...
        for i in 0..10 {
            e.on_edge_observation(&obs("a", 40.0, 2.0, false), i)
                .unwrap();
        }
        // Kill it.
        for i in 10..30 {
            e.on_edge_observation(&obs("a", 0.0, 500.0, true), i)
                .unwrap();
        }
    ...
    fn udp_timeout_feeds_fallback_chain() {
        ...
        for i in 0..10 {
            e.on_edge_observation(&obs("a", 40.0, 3.0, false), i)
                .unwrap();
        }
        // UDP path collapses.
        for _ in 0..8 {
        ...
        assert!(
            e.fallback.current() != TransportKind::Udp
                || !matches!(
                    e.transport_states.get(&TransportKind::Udp).map(|s| s.state),
                    Some(TransportHealth::Healthy)
                )
        );
    }
}
