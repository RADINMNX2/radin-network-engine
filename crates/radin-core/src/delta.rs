...
fn collect_diff(prefix: &str, a: &Map<String, Value>, b: &Map<String, Value>, out: &mut Map<String, Value>) {
...
/// negligible).
pub fn should_apply_delta(full_snapshot: &Value, delta: &Map<String, Value>, min_fields: usize) -> bool {
...
pub fn encode_sync(base: Option<&Value>, next: &Value) -> SyncDelta {
    match base {
        None => SyncDelta { full: Some(next.clone()), patches: None },
        Some(prev) => match diff(prev, next) {
            None => SyncDelta { full: None, patches: None }, // unchanged
            Some(patches) => SyncDelta { full: None, patches: Some(patches) },
        },
    }
}
...
mod tests {
    ...
    fn delta_preferred_below_field_threshold() {
        ...
        assert!(!should_apply_delta(&a, &d, 2), "below min_fields → send full snapshot");
    }
}